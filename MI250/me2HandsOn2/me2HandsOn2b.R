...
modelName <- "me2HandsOn2b" # root names of model file
exampleDir <- "me2HandsOn2"
...
parametersToPlot <- c("ec50.hat", "gamma", "sigma", "omega.ec50")
...
densityplot(~ epeak | ordered(paste(dose, "mg"),
            ...
            panel = function(x, y, subscripts, ...){
              ...
              ltext(x = min(x) + 0.99 * (max(x) - min(x)), y = 0.25, adj = c(1, 0),
...
densityplot(~ eauc | ordered(paste(dose, "mg"),
            ...
            panel = function(x, y, subscripts, ...){
              ...
              ltext(x = min(x) + 0.99 * (max(x) - min(x)), y = 0.015, adj = c(1, 0),
                    labels = paste("Pr(pred >= obs)\n=",
